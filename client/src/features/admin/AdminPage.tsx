...
import {
  Shield, Search, RefreshCw, Trash2, KeyRound, Power, PowerOff,
  ChevronLeft, ChevronRight, MoreHorizontal, Users, TrendingUp,
  CheckCircle2, Lock, Eye, EyeOff, Loader2,
} from 'lucide-react';
...
const PLAN_COLORS: Record<string, string> = {
  FREE:         'bg-surface-400/60 text-slate-400',
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="rounded px-2 py-1 text-[11px] font-semibold text-danger bg-danger/10 hover:bg-danger/20 transition-colors"
        >
      ...
      {open && createPortal(
        <>
          ...
          >
            ...
            <div className="px-3 py-1.5 border-b border-surface-400/30">
              ...
              <div className="mt-1 flex flex-wrap gap-1">
                {PLANS.map((p) => (
                  <button
                    key={p}
                    onClick={() => { planMut.mutate(p); setOpen(false); }}
                    className={cn(
                      'rounded px-2 py-0.5 text-[10px] font-medium transition-colors',
                      user.plan === p ? 'bg-brand-600/30 text-brand-300' : 'text-slate-400 hover:text-slate-200',
                    )}
                  >
                    {p.slice(0, 3)}
                  </button>
            ...
            {/* Role */}
            <div className="px-3 py-1.5 border-b border-surface-400/30">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-600">Role</p>
              <div className="mt-1 flex flex-wrap gap-1">
                {ROLES.map((r) => (
                  <button
                    key={r}
                    onClick={() => { roleMut.mutate(r); setOpen(false); }}
                    className={cn(
                      'rounded px-2 py-0.5 text-[10px] font-medium transition-colors',
                      user.role === r ? 'bg-brand-600/30 text-brand-300' : 'text-slate-400 hover:text-slate-200',
                    )}
                  >
                    {r.replace('_', ' ')}
                  </button>
            ...
            <div className="flex flex-col">
              ...
              <button
                onClick={() => { activeMut.mutate(!user.isActive); setOpen(false); }}
                className="flex items-center gap-2 px-3 py-2 text-xs text-slate-400 hover:bg-surface-300 hover:text-slate-200 transition-colors"
              >
                {user.isActive
...
export default function AdminPage() {
  ...
  const debouncedSearch = useCallback(
    (() => {
      ...
      return (v: string) => {
        clearTimeout(t);
        t = setTimeout(() => { setSearch(v); setPage(1); }, 300);
      };
    })(),
    [],
  );
        ...
        {stats && (
          <div className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-8">
            {[
              ...
              { label: 'Free',           value: stats.byPlan['FREE'] ?? 0,         icon: null },
              ...
              <div key={s.label} className="rounded-xl border border-surface-400/40 bg-surface-100 px-4 py-3 text-center">
            ...
            <tbody className="divide-y divide-surface-400/20">
              {isLoading ? (
                ...
                <tr key={u.id} className={cn('hover:bg-surface-200/40 transition-colors', !u.isActive && 'opacity-50')}>
                  ...
                  <td className="px-4 py-3">
                    <span className={cn('rounded-full px-2 py-0.5 text-[11px] font-semibold', PLAN_COLORS[u.plan] ?? 'text-slate-400')}>
                      {u.plan}
                    </span>
                    {u.trialEndsAt && new Date(u.trialEndsAt) > new Date() && (
                      <p className="mt-0.5 text-[10px] text-amber-400">trial → {timeAgo(u.trialEndsAt).replace(' ago', '')}</p>
                    )}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <p className="text-xs text-slate-500 tabular-nums">
                      {u._count.ownedProperties}p · {u._count.ownedLeases}l · {u._count.ownedTenants}t
                    </p>
                  </td>
                  <td className="px-4 py-3">
                    {u.isActive
                      ? <span className="text-[11px] font-medium text-success">Active</span>
                      : <span className="text-[11px] font-medium text-danger">Suspended</span>
                    }
                  </td>
            ...
            <div className="flex items-center justify-between border-t border-surface-400/30 px-4 py-3 bg-surface-200/30">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={page === 1}
                  className="flex h-7 w-7 items-center justify-center rounded-md border border-surface-400/40 text-slate-400 hover:text-slate-200 disabled:opacity-30 transition-colors"
                >
