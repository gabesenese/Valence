...
export function DatePicker({ value, onChange, onClear, placeholder = 'Select date', disabled, className }: DatePickerProps) {
    ...
    <div ref={ref} className={`relative ${className ?? ''}`}>
      <button
        ...
        className={`flex h-9 w-full items-center gap-2.5 rounded-xl border bg-surface-200 px-3 text-sm transition-all disabled:opacity-50 ${
          open ? 'border-brand-500/60 ring-2 ring-brand-500/10' : 'border-surface-400 hover:border-surface-300'
        } ${label ? 'text-slate-100' : 'text-slate-500'}`}
      >
        <Calendar className="h-3.5 w-3.5 shrink-0 text-slate-500" />
        <span className="flex-1 text-left">{label || placeholder}</span>
        <ChevronDown className={`h-3.5 w-3.5 shrink-0 text-slate-600 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
      </button>

      {open && (
        <div className="absolute left-0 top-full z-50 mt-2 w-[264px] overflow-hidden rounded-2xl border border-surface-400/60 bg-surface-100 shadow-2xl shadow-black/50">
          {/* Month nav */}
          <div className="flex items-center justify-between border-b border-surface-400/30 px-4 py-3">
            ...
            <span className="text-sm font-semibold text-slate-200">{MONTHS[viewMonth]} {viewYear}</span>
            <button
              type="button"
              onClick={() => step(1)}
              className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-500 transition-colors hover:bg-surface-300 hover:text-white"
          ...
          <div className="px-3 pb-3 pt-2">
            <div className="mb-1 grid grid-cols-7">
              {DAY_LABELS.map(d => (
                <div key={d} className="flex h-7 items-center justify-center text-[10px] font-semibold uppercase tracking-wide text-slate-600">
                  {d}
                </div>
              ))}
            </div>
